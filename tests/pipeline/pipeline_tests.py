...
class PipelineTest(unittest.TestCase):
  ...
  def test_get_datetime_expr(self):
    ...
    self.assertEqual(datetime_expr,
                     'datetime.datetime.strptime(\'2009-05-05T22:28:15Z\', \'%Y-%m-%dT%H:%M:%SZ\').replace(tzinfo=timezone(\'UTC\'))')
    self.assertEqual(eval(datetime_expr), datetime.datetime(2009, 5, 5, 22, 28, 15, tzinfo=timezone('UTC')))
