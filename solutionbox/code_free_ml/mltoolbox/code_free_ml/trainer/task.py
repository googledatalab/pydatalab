...
def make_prediction_output_tensors(args, features, input_ops, model_fn_ops,
  ...
  # tf.squeeze with axis=1 is used because each output tensor has shape [1]
  ...
  outputs = {}
  outputs.update({key_name: tf.squeeze(input_ops.features[key_name], axis=[1])
  ...
  if is_classification_model(args.model):
    ...
    if keep_target:
      ...
      outputs[PG_TARGET] = tf.squeeze(input_target_label, axis=[1])
    ...
    if keep_target:
      outputs[PG_TARGET] = tf.squeeze(input_ops.features[target_name], axis=[1])

    scores = model_fn_ops.predictions['scores']
    outputs[PG_REGRESSION_PREDICTED_TARGET] = tf.squeeze(scores, axis=[1])
