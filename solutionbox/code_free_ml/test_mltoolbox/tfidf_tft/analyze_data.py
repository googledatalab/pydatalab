...
def make_split_str_to_int_tito(vocab, default_value):
  def _str_to_int(x):
...
def make_tfidf_titovvvvvvvvv(vocab, example_count, corpus_size, part):
  ...
  def _tfidf(x):
    ...
    dense_shape = int_text.dense_shape # Note the number of cols is an upperbound.
    #dense_shape = [x.shape[0], len(vocab)]
    #dense_shape = tf.Print(dense_shape, [dense_shape])
...
def make_tfidf_tito2(vocab_size_with_default, example_count, corpus_size, part):
  def _tfidf(x):
    int_text = x

    term_count_per_doc = get_term_count_per_doc(int_text, vocab_size_with_default)
...
def make_bag_of_words_tito(vocab, part):
  def _bow(x):
    ...
    indices = tf.stack([term_count_per_doc.indices[:, 0],
                       ...
                       1)
    #dense_shape = term_count_per_doc.dense_shape
    dense_shape = int_text.dense_shape
    ...
    for name, transform in six.iteritems(features):
      ...
      if transform_name == 'identity':
        ...
        if transform_name == TFIDF_TRANSFORM:
          tokens = tft.map(make_split_str_to_int_tito(vocab=vocab, default_value=len(vocab)),
                           inputs[name])
          result[name + '_ids'] = tft.map(
              make_tfidf_tito2(
                              vocab_size_with_default=len(vocab) +1,
...
def make_transform_graph(output_dir, schema, features):
  ...
  with beam.Pipeline('DirectRunner', options=None) as p:
    with tft_impl.Context(temp_dir=temp_dir):
      dummy_data = p | beam.Create([{'target': '101', 'key': 23, 'str_tfidf': 'hello world'}])
      (train_dataset, train_metadata), transform_fn = (
        (dummy_data, tft_input_metadata)
